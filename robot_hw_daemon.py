#!/usr/bin/env python3

import rclpy
from rclpy.node import Node
import time
import subprocess
import sys
from sensor_msgs.msg import BatteryState
from irobot_create_msgs.msg import LightringLeds, LedColor, AudioNoteVector, AudioNote, InterfaceButtons, DockStatus
from builtin_interfaces.msg import Duration
import os

class RobotController(Node):
    def __init__(self):
        # Get robot namespace from stored file
        robot_id = self.get_robot_id()
        namespace = f"robot_{robot_id}"
        super().__init__('robot_hw_daemon', namespace=namespace)

        # --- Path for TMUX Status ---
        self.status_file = "/tmp/robot_status"

        # --- Publishers ---
        self.lightring_pub = self.create_publisher(LightringLeds, 'cmd_lightring', 10)
        self.audio_pub = self.create_publisher(AudioNoteVector, 'cmd_audio', 10)

        # --- Subscriptions ---
        self.battery_sub = self.create_subscription(
            BatteryState, 'battery_state', self.battery_callback, 10)
        
        self.dock_sub = self.create_subscription(
            DockStatus, 'dock_status', self.dock_callback, 10)
        
        self.button_sub = self.create_subscription(
            InterfaceButtons, 'interface_buttons', self.button_callback, 10)

        # --- State Variables ---
        self.startup_done = False
        self.press_count = 0
        self.last_button_state = False
        self.last_press_time = 0.0
        
        # --- Battery & Dock Variables ---
        self.current_battery_percentage = 1.0 
        self.is_docked = False
        self.battery_threshold = 0.20          
        
        # --- Config ---
        self.python_executable = sys.executable
        self.target_script = "/home/ubuntu/robot_setup/change_wifi.py"
        self.target_args = "--ap --force"
        
        self.required_presses = 5
        self.button_timeout = 3.0 

        # --- Timers ---
        self.battery_alert_timer = self.create_timer(300.0, self.check_battery_alert)

        self.get_logger().info(f'Jazzy Manager [{namespace}] Active: Monitoring Hardware...')
    
    def get_robot_id(self):
        id_file = "/home/ubuntu/.turtlebot_id"
        if os.path.exists(id_file):
            with open(id_file, 'r') as f:
                return f.read().strip()
        return "XX"

    def update_status_file(self):
        """Generates the string for the TMUX status bar and writes it to a file."""
        try:
            # Battery Icon & Percent
            percent = int(self.current_battery_percentage * 100)
            if percent >= 90: batt_icon = "󰁹"
            elif percent >= 70: batt_icon = "󰂀"
            elif percent >= 50: batt_icon = "󰁾"
            elif percent >= 30: batt_icon = "󰁼"
            else: batt_icon = "󰂃"
            
            # Dock Status
            dock_str = "󱐋 Charging" if self.is_docked else "󰑭 Mobile"
            
            # Short Timestamp (HH:MM)
            t_stamp = time.strftime("%H:%M")
            
            # Combine for TMUX: Icon Percent | Status | Time
            full_str = f"{batt_icon} {percent}% │ {dock_str} │ {t_stamp}\n"
            
            with open(self.status_file, 'w') as f:
                f.write(full_str)
        except Exception as e:
            # We use pass to avoid flooding the log if the file system is briefly busy
            pass

    def dock_callback(self, msg):
        self.is_docked = msg.is_docked
        self.update_status_file()

    def battery_callback(self, msg):
        self.current_battery_percentage = msg.percentage
        self.update_status_file()

        if not self.startup_done:
            self.startup_done = True
            self.play_startup_sound()
            self.run_circle_animation(color="pink", duration_sec=4.0)

    def check_battery_alert(self):
        """Muted if docked, otherwise beeps/flashes red below 20%."""
        if not self.is_docked and self.current_battery_percentage <= self.battery_threshold:
            self.play_low_battery_sound()
            self.run_circle_animation(color="red", duration_sec=2.0)

    def button_callback(self, msg):
        current_state = msg.button_1.is_pressed
        if current_state and not self.last_button_state:
            now = time.time()
            if now - self.last_press_time > self.button_timeout:
                self.press_count = 1
            else:
                self.press_count += 1
            self.last_press_time = now

            if self.press_count == self.required_presses:
                self.play_long_beep()
                self.run_circle_animation(color="green", duration_sec=5.0)
                self.execute_wifi_script()
                self.press_count = 0 
        self.last_button_state = current_state

    # --- Audio & LED Utilities ---
    def play_startup_sound(self):
        if self.wait_for_audio_subscriber():
            msg = AudioNoteVector()
            msg.header.stamp = self.get_clock().now().to_msg()
            msg.notes = [
                AudioNote(frequency=659, max_runtime=Duration(sec=0, nanosec=150000000)),
                AudioNote(frequency=784, max_runtime=Duration(sec=0, nanosec=150000000)),
                AudioNote(frequency=880, max_runtime=Duration(sec=0, nanosec=150000000)),
                AudioNote(frequency=784, max_runtime=Duration(sec=0, nanosec=300000000)),
            ]
            self.audio_pub.publish(msg)

    def play_low_battery_sound(self):
        msg = AudioNoteVector()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.notes = [
            AudioNote(frequency=440, max_runtime=Duration(sec=0, nanosec=200000000)),
            AudioNote(frequency=330, max_runtime=Duration(sec=0, nanosec=400000000)),
        ]
        self.audio_pub.publish(msg)

    def play_long_beep(self):
        msg = AudioNoteVector()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.notes = [AudioNote(frequency=500, max_runtime=Duration(sec=2, nanosec=0))]
        self.audio_pub.publish(msg)

    def wait_for_audio_subscriber(self):
        for _ in range(20):
            if self.audio_pub.get_subscription_count() > 0: return True
            time.sleep(0.5)
        return False

    def reset_leds(self):
        msg = LightringLeds(override_system=False)
        msg.header.stamp = self.get_clock().now().to_msg()
        self.lightring_pub.publish(msg)

    def run_circle_animation(self, color, duration_sec):
        start_time = time.time()
        led_index = 0
        target_color = LedColor(red=255, green=20, blue=147) if color == "pink" else \
                       LedColor(red=0, green=255, blue=0) if color == "green" else \
                       LedColor(red=255, green=0, blue=0)
        off = LedColor(red=0, green=0, blue=0)

        while (time.time() - start_time) < duration_sec:
            msg = LightringLeds(override_system=True)
            msg.header.stamp = self.get_clock().now().to_msg()
            leds = [off] * 6
            leds[led_index] = target_color
            msg.leds = leds
            self.lightring_pub.publish(msg)
            led_index = (led_index + 1) % 6
            time.sleep(0.06)
        self.reset_leds()

    def execute_wifi_script(self):
        try:
            cmd = f"sudo {self.python_executable} {self.target_script} {self.target_args}"
            subprocess.Popen(cmd, shell=True)
        except Exception as e:
            self.get_logger().error(f'Failed WiFi script: {e}')
    
def main(args=None):
    rclpy.init(args=args)
    node = RobotController()
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        rclpy.shutdown()

if __name__ == '__main__':
    main()